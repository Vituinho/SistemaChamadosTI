...
export function TicketDetails({ ticket, accessKey }: { ticket: Ticket; accessKey?: string }) {
  ...
  return <div className="space-y-6">
    ...
    <div><h3 className="font-semibold">Descrição</h3><p className="mt-2 whitespace-pre-wrap break-words text-sm leading-6 text-slate-600">{ticket.description}</p></div>
    ...
    <div><h3 className="mb-4 font-semibold">Histórico do atendimento</h3><ol className="space-y-4 border-l-2 border-orange-100 pl-4">{ticket.history.map(h => <li key={h.id}><p className="text-xs text-slate-500">{date(h.created_at)}</p><p className="mt-1 whitespace-pre-wrap break-words text-sm">{h.message}</p></li>)}</ol></div>
  </div>;
}
