...
export function TicketDetails({ ticket, accessKey }: { ticket: Ticket; accessKey?: string }) {
  ...
  return <div className="space-y-7">
    ...
    <section aria-labelledby="requester-info"><h3 id="requester-info" className="mb-4 flex items-center gap-2 font-bold"><UserRound className="size-5 text-orange-600" /> Solicitante</h3><dl className="grid gap-4 rounded-xl border border-slate-200 p-4 sm:grid-cols-2"><Info label="Nome" value={ticket.name} /><Info label="Setor" value={ticket.department} /><Info label="Estado" value={ticket.state} /><Info label="Localização" value={ticket.location} /></dl></section>
