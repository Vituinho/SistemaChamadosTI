...
function Panel({ user, onLogout }: { user: User; onLogout: () => void }) {
  ...
  const [filters, setFilters] = useState({ status: "", department: "", category: "", affected_system: "", priority: "", q: "" });
  ...
  return <main className="mx-auto max-w-7xl space-y-6 px-4 py-7 sm:px-6 sm:py-10">
    ...
    <section className="card"><div className="mb-5 flex flex-wrap items-center justify-between gap-2"><h2 className="text-xl font-bold">Fila de chamados</h2><span className="text-xs text-slate-500">Mais antigos primeiro · {total} encontrados</span></div>
      <div className="mb-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-6"><label className="sm:col-span-2 xl:col-span-1"><span className="flex items-center gap-1"><Search size={14} />Buscar</span><input value={filters.q} onChange={e => filter("q", e.target.value)} placeholder="Protocolo, nome, título ou descrição" maxLength={160} /></label>{([ ["status", "Status", catalog?.statuses], ["priority", "Prioridade", catalog?.priorities], ["department", "Setor", catalog?.departments], ["category", "Categoria", catalog?.categories], ["affected_system", "Sistema", catalog?.systems] ] as const).map(([key, label, values]) => <label key={key}>{label}<select value={filters[key]} onChange={e => filter(key, e.target.value)}><option value="">Todos</option>{values?.map(v => <option key={v} value={v}>{labels[v] || v}</option>)}</select></label>)}</div>
      {!loaded ? <div role="status" aria-label="Carregando chamados" className="space-y-3 py-2">{[1, 2, 3].map(item => <div key={item} className="skeleton h-28" />)}</div> : tickets.length === 0 ? <div className="py-12 text-center"><Inbox className="mx-auto mb-3 text-emerald-500" /><h3 className="font-semibold">Nenhum chamado aguardando atendimento 🎉</h3><p className="mt-2 text-sm text-slate-500">Novas solicitações aparecerão automaticamente. Se esperava algum resultado, limpe os filtros.</p></div> : <div className="space-y-3">{tickets.map(t => <button key={t.id} onClick={() => openTicket(t.id)} className={`w-full rounded-xl border p-4 text-left transition hover:border-orange-400 hover:shadow-sm focus-visible:outline-2 focus-visible:outline-orange-600 ${t.status === "NOVO" ? "border-orange-300 bg-orange-50/70 shadow-sm" : "border-slate-200 bg-white"}`}><div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><span className="text-xs font-black text-orange-700">{t.protocol}</span><h3 className="mt-1 break-words font-bold text-slate-900">{t.title}</h3></div><div className="flex flex-wrap gap-2"><Badge value={t.status} /><Badge value={t.priority} /></div></div><div className="mt-3 grid gap-1 text-sm text-slate-600 sm:grid-cols-2"><p className="break-words"><strong className="text-slate-800">{t.name}</strong> · {t.department}</p><p className="break-words sm:text-right">{t.location || "Local não informado"}</p></div><div className="mt-3 flex flex-wrap justify-between gap-2 border-t border-slate-200 pt-3 text-xs text-slate-500"><span>{t.affected_system || t.category} · {t.technician || "Sem responsável"}</span><span>{date(t.created_at)} · esperando há {age(t.created_at)}</span></div></button>)}</div>}
