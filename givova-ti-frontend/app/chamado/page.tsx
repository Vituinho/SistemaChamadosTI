...
import { Check, Database, Headset, HelpCircle, Layers3, LoaderCircle, MapPin, Paperclip, Send, Truck, UserRound, X } from "lucide-react";
...
export default function Chamado() {
  ...
  async function enviarChamado(event: FormEvent<HTMLFormElement>) {
    ...
    if (!String(data.get("state") || "")) { showError("Escolha o estado onde você está."); return; }
    ...
    {!catalog ? <div role="status" className="card space-y-4 py-10"><div className="flex items-center gap-3 font-semibold"><LoaderCircle className="animate-spin text-orange-600 motion-reduce:animate-none" /> Preparando o formulário…</div><div className="skeleton h-12" /><div className="skeleton h-28" /></div> : <form onSubmit={enviarChamado}>
      <fieldset disabled={busy} className="min-w-0 space-y-5"><legend className="sr-only">Abrir chamado para a TI</legend>
        <section className="card space-y-5" aria-labelledby="who-heading">
          ...
          <div className="grid gap-5 sm:grid-cols-3"><label htmlFor="requester-name" className="text-base sm:col-span-2">Nome completo<input id="requester-name" name="name" autoComplete="name" required minLength={2} maxLength={120} placeholder="Como podemos chamar você?" /></label><label htmlFor="requester-state" className="text-base">Estado<select id="requester-state" name="state" defaultValue="" required><option value="" disabled>Selecione</option>{catalog.states.map(state => <option key={state} value={state}>{state}</option>)}</select></label><label htmlFor="requester-location" className="text-base sm:col-span-3">Onde você está?<span className="mt-1 block text-xs font-normal text-slate-500">Sala, unidade ou ponto de referência</span><div className="relative"><MapPin className="pointer-events-none absolute top-[21px] left-3 size-4 text-slate-400" /><input id="requester-location" name="location" required minLength={2} maxLength={160} className="pl-9" placeholder="Ex.: sala do Financeiro" /></div></label></div>
        ...
        <section className="card space-y-5" aria-labelledby="problem-heading">
          ...
          <fieldset><legend className="mb-3 text-base font-semibold">Com o que precisa de ajuda?</legend><div className="grid grid-cols-2 gap-2 sm:grid-cols-3">{catalog.categories.map(c => { const Icon = problemOptions[c]?.icon || HelpCircle; return <label key={c} className="relative cursor-pointer"><input className="peer sr-only" type="radio" name="category" value={c} checked={category === c} onChange={() => chooseCategory(c)} required /><span className="flex min-h-16 items-center gap-3 rounded-xl border border-slate-300 bg-white px-3 py-3 text-sm peer-checked:border-orange-600 peer-checked:bg-orange-50 peer-checked:text-orange-800 peer-focus-visible:outline-2 peer-focus-visible:outline-offset-2 peer-focus-visible:outline-orange-600 hover:border-orange-400"><Icon className="size-6 shrink-0 text-orange-700" aria-hidden="true" /><span className="flex-1">{c === "Outro" ? "Outro / Não sei" : c}</span>{category === c && <Check size={17} className="shrink-0" aria-hidden="true" />}</span></label>; })}<button type="button" onClick={() => chooseSystem("DBFrete")} aria-pressed={category === "Sistema" && affectedSystem === "DBFrete"} className={`flex min-h-16 items-center gap-3 rounded-xl border bg-white px-3 py-3 text-left text-sm transition focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-orange-600 ${category === "Sistema" && affectedSystem === "DBFrete" ? "border-orange-600 bg-orange-50 text-orange-800" : "border-slate-300 hover:border-orange-400"}`}><Database className="size-6 shrink-0 text-orange-700" aria-hidden="true" /><span className="flex-1 font-semibold">DBFrete</span>{category === "Sistema" && affectedSystem === "DBFrete" && <Check size={17} aria-hidden="true" />}</button><button type="button" onClick={() => chooseSystem("Frete Brás")} aria-pressed={category === "Sistema" && affectedSystem === "Frete Brás"} className={`flex min-h-16 items-center gap-3 rounded-xl border bg-white px-3 py-3 text-left text-sm transition focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-orange-600 ${category === "Sistema" && affectedSystem === "Frete Brás" ? "border-orange-600 bg-orange-50 text-orange-800" : "border-slate-300 hover:border-orange-400"}`}><Truck className="size-6 shrink-0 text-orange-700" aria-hidden="true" /><span className="flex-1 font-semibold">Frete Brás</span>{category === "Sistema" && affectedSystem === "Frete Brás" && <Check size={17} aria-hidden="true" />}</button></div></fieldset>
